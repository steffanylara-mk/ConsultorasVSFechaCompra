...
with tab_rec:
    ...
    if n_target == 0:
        ...
        st.markdown(
            ...
            f"El CSV de descarga incluye ConsultantNumber para uso operativo."
            f"</div>",
            unsafe_allow_html=True,
        )

        # Render con tres niveles de fallback:
        # 1) Styler con background_gradient (lo bonito)
        # 2) Styler solo con formato (sin gradient, mas robusto en Arrow)
        # 3) DataFrame con columnas pre-formateadas como strings (siempre funciona)
        _rendered = False

        # Nivel 1: Styler con gradient
        try:
            ...
            try:
                ...
                pass
            st.dataframe(_s, use_container_width=True, height=520)
            _rendered = True
        except Exception:
            pass

        # Nivel 2: Styler sin gradient
        if not _rendered:
            try:
                _s2 = ui_view.style.format(_fmt_map)
                st.dataframe(_s2, use_container_width=True, height=520)
                _rendered = True
            except Exception:
                pass

        # Nivel 3: pre-formato a string y dataframe plano
        if not _rendered:
            _display = ui_view.copy()
            for _c in ["WholesaleAcumulado", "TicketPromedio"]:
                if _c in _display.columns:
                    _display[_c] = _display[_c].map(_fmt_money)
            for _c in ["MesesConOrden", "TotalOrdenes"]:
                if _c in _display.columns:
                    _display[_c] = _display[_c].map(_fmt_int)
            # Asegurar que TODAS las columnas restantes sean strings o numericas limpias
            for _c in _display.columns:
                if _display[_c].dtype == object:
                    _display[_c] = _display[_c].astype(str).replace({"nan": "", "None": ""})
